...
- 功能：批量收集多支股票的K線數據，並加入時間感知的更新策略，避免在盤前/盤後重複拉資料。
- 關鍵判斷邏輯：
  - 讀取 `Data/kbar/{stock}_Raw.csv` 的最後一筆 `ts`，用於判斷該檔案是否已含前/當日 13:30 的收盤列。
  - **盤前 (09:00 前)**：若 Raw 檔最後時間戳 ≥ 前一個交易日且時間 ≥ 13:30，視為資料已ready，直接跳過下載。
  - **盤中**：維持原有冷卻機制（5 分鐘內不重複請求），若 `daily` CSV 尚未更新到當日即刻，則允許補抓。
  - **盤後 (13:30 之後)**：若 Raw 檔已寫入當日 13:30 之列，代表今日資料完整，不再重新請求。
  - 若 Raw 仍缺當日（或最近交易日）的收盤列，且符合時間條件，才會觸發 `get_stock_kbars()`。
- 主要組件：
  - `collect_and_save_kbars()`：根據配置文件收集並保存K線數據，並包含上述時間窗口檢查。
  - `_get_last_raw_timestamp()` / `_was_file_updated_after_cutoff()`：輔助判斷 Raw 或 Daily 檔是否已於 13:40 後更新完畢。
...
> 買入規則的詳細條件已獨立整理於 `BUY_RULES_SPEC.md`，若需查詢每一條買訊邏輯、欄位與輸入要求，請參考該文件。
